        ...
        tag("main")(cls := "strategy-shell")(
          header(cls := "strategy-hero")(
            ...
            p(cls := "strategy-intro")(
              "Choose four classes in fixed front-to-rear order, then select a boss for a tailored fight plan."
            ),
            div(cls := "catalog-badges", attr("aria-label") := "Strategy catalog")(
              span(cls := "catalog-badge")("6 classes"),
              span(cls := "catalog-badge")("29 party rules"),
              span(cls := "catalog-badge")("18 bosses"),
              span(cls := "catalog-badge")("69 boss rules")
            )
          ),
          tag("section")(cls := "party-builder", attr("aria-labelledby") := "party-builder-title")(
            div(cls := "section-heading")(
              div(
                ...
                h2(id := "party-builder-title")("Four fixed positions. One shared plan.")
              ),
              p(cls := "section-note")("Slot 1 is front · full class toolkit assumed")
            ),
            form(id := "strategy-controls", cls := "strategy-controls")(
              (1 to 4).map: slot =>
                val slotLabel = if slot == 1 then "Front member" else s"Party member $slot"
                label(cls := s"party-slot party-slot-$slot")(
                  span(cls := "slot-number")(f"$slot%02d"),
                  span(cls := "slot-label")(slotLabel),
                  select(
                    name                    := "party-member",
